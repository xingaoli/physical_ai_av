...
def load_physical_aiavdataset(
    ...
    extr = avdi.get_clip_feature(clip_id, "sensor_extrinsics", maybe_stream=True)
    intr = avdi.get_clip_feature(clip_id, "camera_intrinsics", maybe_stream=True)
    vehicle_dimensions = avdi.get_clip_feature(clip_id, "vehicle_dimensions", maybe_stream=True)
    ...
    for cam_feature in camera_features:
        ...
        if cam_idx == 1:
            front_wide_frame = frames[-1].copy()
    ...
    return {
        ...
        # "front_wide_frame": front_wide_frame,  # (H, W, 3) uint8 numpy, current frame at t0
    }
...
def main():
    ...
    if args.debug:
        def has_valid_event(events_str):
            if not isinstance(events_str, str):
                return False
            events = json.loads(events_str)
            return any(e["event_start_timestamp"] > MIN_T0_US for e in events)
        df = df[df["events"].apply(has_valid_event)].head(5)
        print(f"[debug] Limiting to {len(df)} clips (with t0 > {MIN_T0_US / 1e6:.1f}s)")
    ...
    for clip_id, row in df.iterrows():
        ...
        if not isinstance(events_str, str):
            continue
        for event in json.loads(events_str):
            t0_us = event["event_start_timestamp"]
            if t0_us > MIN_T0_US:
                pending.append((clip_id, t0_us, event))
    ...
    while pending:
        ...
        for clip_id, t0_us, event in tqdm(pending, desc=f"Round {round_num} ({len(pending)} tasks)"):
            try:
                ...
                tqdm.write(f"Error clip={clip_id}, t0={t0_us}: {e}")
    ...
    remaining = len(pending) if pending else 0
    if remaining:
        print(f"WARNING: {remaining} tasks still failed after all retries.")
    else:
        print(f"Done. Total tasks completed: {total}")
